...
class ChatServer:
    ...
    def __init__(self):
        ...
        self.tls_context.load_cert_chain(certfile="server.crt", keyfile="server.key")
    ...
    def receive_messages_in_a_new_thread(self):
        while True:
            plain_sock, (ip, port) = self.server_socket.accept() #소켓 연결 수락
            so = self.tls_context.wrap_socket(plain_sock, server_side=True) # TLS 래핑
            client = so, (ip, port)
